...
import com.sj.yinjiaoyun.shituwang.activity.TeachterChatActivity;
 ...
 * 老师端的会话列表的适配器
 */
public class TeachterMessageListAdapter extends RecyclerView.Adapter  {
    ...
    public TeachterMessageListAdapter(Context context, List<RecentlyMsgBean.DataBean> list) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.teachter_message_list_item, parent, false);
        return new TeachterMessageHolder(view);
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        final TeachterMessageHolder messageHolder = (TeachterMessageHolder) holder;
        ...
        if(dataBean.getMsgLogo()!=null&& !TextUtils.isEmpty(dataBean.getMsgLogo())){
            ...
            PicassoUtils.LoadCorners(context, R.drawable.wukong, 60, 60, messageHolder.circleView);
        }
        //展示消息名字
        ...
        //展示性别
        if(!TextUtils.isEmpty(dataBean.getSex()) && "0".equals(dataBean.getSex())){
            messageHolder.ivSex.setImageResource(R.mipmap.woman);
        }else {
            messageHolder.ivSex.setImageResource(R.mipmap.male);
        }
        //展示消息时间
...
//            T42：老师取消面试
            switch (type){
                ...
                case "T02":
                    ...
                    break;
                case  "T03":
                    msg="您设置"+dataBean.getMsgName()+"为不合适";
                    break;
                case "T04":
                    msg="您取消了"+dataBean.getMsgName()+"不合适的设置";
                    break;
                case  "T10":
                    msg="交换电话请求已发送";
                    break;
    ...
    private void getTeacherDetail(final RecentlyMsgBean.DataBean dataBean) {
        ...
        map.put("id",dataBean.getReceiver().split("_")[1]);
    ...
    private void getStudentRecord(final RecentlyMsgBean.DataBean dataBean) {
        String params = "?studentId=" + String.valueOf(dataBean.getSender().split("_")[1]);
        HttpClient.get(this, Api.GTE_STUDENT_RECORD + params, new CallBack<StudentRecordBean>() {
            ...
            public void onSuccess(StudentRecordBean result) {
                ...
                if (result.getStatus() == 200  && result.getData() != null) {
                    TeachterChatActivity.StartActivity(context,dataBean.getSender(),teacherDetailBean.getData(),result.getData(),String.valueOf(dataBean.getUserContactId()));
                } else {
                    ToastUtil.showShortToast(context, result.getMsg());
                }
    ...
    class TeachterMessageHolder extends RecyclerView.ViewHolder {
        ...
        public TeachterMessageHolder(View view) {
            super(view);
