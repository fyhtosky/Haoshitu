...
import com.sj.yinjiaoyun.shituwang.adapter.StuEvaAdapter;
 ...
 * 跟师学习的已终止的Fragment
 */
public class WithTeachterTerminatedFragment extends Fragment {
    @BindView(R.id.rv)
    RecyclerView rv;
    ...
    //适配器
    private StuEvaAdapter stuEvaAdapter;
    ...
    private void init() {
        ...
        stuEvaAdapter = new StuEvaAdapter(list, getContext());
        //添加适配器
        rv.setAdapter(stuEvaAdapter);
        queryStuEva();
    }
    ...
    private void queryStuEva() {
        ...
        HttpClient.post(this, Api.QUEERY_STU_EVAPG, map, new CallBack<StuEvaBean>() {
            @Override
            public void onSuccess(StuEvaBean result) {
                ...
                if (result.getData() != null && result.getData().getRows() != null) {
                    for (int i = 0; i < result.getData().getRows().size(); i++) {
                        if (result.getData().getRows().get(i).getStatus() == 2) {
                    ...
                    stuEvaAdapter.notifyDataSetChanged();
                }
